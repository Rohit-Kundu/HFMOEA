...
parser.add_argument('--mutation', type=int, default = 10, help='Percentage of mutation in the Genetic Algorithm')
...
topk = args.topk
sol_per_pop = len(sol)
initial_chromosome = np.zeros(shape=(sol_per_pop,data.shape[1]))
...
#The main program
sol_per_pop = len(sol) # Population size
...
for train_index, test_index in kfold.split(data_inputs):
    ...
    for generation in range(num_generations):
        ...
        print(matrix_test)
        print("Classwise Accuracy :{}".format(matrix_test.diagonal()/matrix_test.sum(axis = 1)))
        '''
        ...
        if best_outputs_test[-1]>test: #and balanced>=0.99*acc:
            best_match_idx = np.where(fitness == np.max(fitness))[0]
            best_match_idx = best_match_idx[0]
...
#print("Average Accuracy = ", np.mean(np.asarray(fold_acc_list)))
